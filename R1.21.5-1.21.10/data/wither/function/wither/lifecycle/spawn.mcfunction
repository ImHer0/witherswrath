...
execute as @e[type=minecraft:wither] run data merge entity @s {attributes:[{id:"minecraft:flying_speed",base:1}]}
execute as @e[type=minecraft:wither] at @s if data storage wither:options {toggleanimation:Default} run summon creeper ~ ~ ~ {DeathLootTable: "wither:entities/withermobs",Team:"Wither",Silent:1b,Invulnerable:1b,CustomNameVisible:0b,ExplosionRadius:13b,Fuse:1,ignited:1b,CustomName:"the Wither's Wrath"}
