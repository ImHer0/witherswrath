...
execute at @e[type=minecraft:wither_skull,tag=buff,limit=1] run summon creeper ~ ~-1 ~ {ExplosionRadius:2b,Fuse:0,Tags:["boom"],CustomName:'"wither skull"',active_effects:[{id:"minecraft:wither",amplifier:1,duration:4}],attributes:[{id:"minecraft:generic.scale",base:0.09}]}
