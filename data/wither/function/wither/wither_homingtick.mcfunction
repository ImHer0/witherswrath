...
# Homing Explosion
execute as @e[type=minecraft:armor_stand,limit=1,sort=nearest,tag=hStand] at @s if entity @e[type=player,distance=..0.45] run summon creeper ~ ~ ~ {Tags: ["hboom"], CustomName: '"Homing Skull"', Fuse: 0, ExplosionRadius: 2, Silent: 1b, attributes:[{id:"minecraft:generic.scale",base:0.09}]}
...
execute as @e[type=minecraft:armor_stand,limit=1,sort=nearest,tag=hStand] at @s anchored eyes positioned ^ ^ ^ unless block ^ ^ ^1 air run summon creeper ~ ~ ~ {Tags: ["hboom"], CustomName: '"Homing Skull"', Fuse: 0, ExplosionRadius: 2, Silent: 1b, attributes:[{id:"minecraft:generic.scale",base:0.09}]}
...
execute as @e[type=minecraft:creeper,tag=hboom] at @s run playsound minecraft:entity.generic.explode hostile @a[distance=..5] ~ ~ ~ 1 1.7 1
