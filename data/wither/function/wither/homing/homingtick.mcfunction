...
# Homing Explosion
execute as @e[type=minecraft:armor_stand,limit=1,sort=nearest,tag=hStand] at @s if entity @e[type=player,distance=..0.45] run summon creeper ~ ~ ~ {Tags: ["hboom"], CustomName: '"Homing Skull"', Fuse: 0, ExplosionRadius: 2, Silent: 1b, attributes:[{id:"minecraft:scale",base:0.09}]}
