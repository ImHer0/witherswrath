...
# GO UP GO DOWN 
execute if data storage wither:options {toggleanimation:Default} at @e[type=wither,limit=1,sort=nearest] align xyz positioned ~.5 ~.5 ~.5 run summon armor_stand ~ ~ ~ {Team:"Wither",Invisible:1b,Tags:["midpointpos"],Attributes:[{Name:generic.scale,Base:2}]}
