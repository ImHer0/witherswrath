...
# Homing Explosion
execute as @e[type=minecraft:armor_stand,limit=1,sort=nearest,tag=hStand] at @s if entity @e[type=player,distance=..0.45] run summon creeper ~ ~ ~ {Team:"Wither",Tags: ["hboom"], CustomName: '{"text":"Homing Skull"}', Fuse: 0, ExplosionRadius: 2, Silent: 1b, }
...
execute as @e[type=minecraft:armor_stand,limit=1,sort=nearest,tag=hStand] at @s anchored eyes positioned ^ ^ ^ unless block ^ ^ ^1 air run summon creeper ~ ~ ~ {Team:"Wither",Tags: ["hboom"], CustomName: '{"text":"Homing Skull"}', Fuse: 0, ExplosionRadius: 2, Silent: 1b, }
