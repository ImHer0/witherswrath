...
execute if data storage wither:options {toggleanimation:Default} as @e[type=item,limit=1,sort=nearest,nbt={Item:{id:"minecraft:nether_star",Count:1b}}] at @s run summon armor_stand ~ ~ ~ {Invisible:1b,Invulnerable:1b,NoGravity:1b,Tags:["starxp"]}
...
# EFFECTS
execute as @e[type=item,sort=nearest,tag=nether_star,limit=1] as @s at @s run playsound minecraft:entity.wither.death hostile @a ~ ~ ~ 1 1
